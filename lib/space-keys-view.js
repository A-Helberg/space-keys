...
export default class spaceKeysView {
  ...
  reload() {
    ...
    const defaultConfFile = path.resolve(__dirname, '..', 'templates', 'space-keys.cson')
    ...
    try {
      ...
      atom.notifications.addWarning(`Could not load your space-keys config`);
    }

    try {
      this.defaultConf = readFileSync(defaultConfFile)
    } catch (ex) {
      atom.notifications.addWarning(`Could not load default space-keys config`);
    }
  }
  ...
  keyListener (keypress) {
    ...
    if (this.keyState === null) {
      if(this.conf){
        ...
        this.keyState = this.defaultConf[charKey]
      }
    } else {
